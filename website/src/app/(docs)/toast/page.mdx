...
Trigger a tactile moment. Use ``toast`()` to communicate with your users through both sight and touch. It requires a mounted [`<Toaster />`](/toaster) and `import 'web-haptics-toast/dist/styles.css'` at your app root.

By default, every toast pulses a haptic pattern on supported devices. If you want to keep a moment quiet, just pass `haptics: false`.
...
| dedupeKey         |                       When enabled in `<Toaster system={...} />`, toasts with the same `dedupeKey` update the existing toast.                       |            `-` |
| haptics           |                       When `false`, skips haptic feedback for this toast only (toaster `haptics` must still be enabled).                        |            `-` |
| onDismiss         |       The function gets called when either the close button is clicked, or the toast is swiped.        |            `-` |
| onClose           |                 Called once when the toast is fully closed, including a dismissal `reason`.                 |            `-` |
| onAutoClose       | Function that gets called when the toast disappears automatically after it's timeout (duration` prop). |            `-` |
| unstyled | Removes all default CSS for total creative freedom. | `false` |
| richColors | Transforms toasts into vibrant, intent-shaping signals. | `false` |
