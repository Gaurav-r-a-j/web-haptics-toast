...
/** Shared link style for footer columns: normal weight, sentence case, calm. */
const footerLinkClass =
  'text-sm font-normal text-white/80 no-underline transition-colors hover:text-secondary tracking-normal';

export const Footer = () => {
  const year = new Date().getFullYear();

  return (
    <footer className="mt-auto bg-primary pt-24 pb-0 overflow-hidden text-white relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          <div className="lg:col-span-4 flex flex-col justify-start">
            <div className="flex flex-col gap-1">
              <HeroText layers={6} className="text-4xl md:text-5xl lg:text-6xl mb-0 text-secondary leading-[0.85] uppercase tracking-tighter">
                WEB HAPTICS
              </HeroText>
              <HeroText layers={6} className="text-4xl md:text-5xl lg:text-6xl mb-0 text-white leading-[0.85] uppercase tracking-tighter">
                TOAST
              </HeroText>
            </div>
            <div className="pt-8">
              <a
                ...
                className="text-2xl font-semibold tracking-tight text-white hover:text-secondary transition-colors no-underline"
              >
                © DesignByte Studio
              </a>
              <p className="m-0 mt-4 max-w-sm text-sm font-normal not-italic leading-relaxed text-white/80">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-3 gap-12 text-left items-start">
            <div className="flex flex-col gap-4">
              <p className="m-0 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/70">
                Docs
              </p>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="pt-4">
                <a href={LINKS.STUDIO} target="_blank" rel="noopener noreferrer" className="inline-block rounded-xl bg-black/20 px-4 py-2 text-[10px] font-semibold uppercase tracking-wide text-white transition-colors hover:bg-black/30">
                  @DESIGNBYTE
                </a>
              </div>
            </div>
      ...
      <div className="bg-black/20 py-6 md:py-8">
        <div className="container mx-auto flex flex-col items-center justify-between gap-6 px-3 sm:px-4 md:flex-row md:gap-8 md:px-8">
          <p className="m-0 text-[11px] md:text-xs font-normal uppercase tracking-wide text-white/80">
            ...
            <div key={group} className="flex items-center">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex items-center gap-6 md:gap-12 px-6 md:px-12">
                  <HeroText layers={8} shadowColor="#000000" className="text-[120px] md:text-[200px] uppercase tracking-tighter leading-[0.85] text-secondary">
                    WEB
                  </HeroText>
                  <HeroText layers={8} shadowColor="#000000" className="text-[120px] md:text-[200px] uppercase tracking-tighter leading-[0.85] text-white">
                    HAPTICS
                  </HeroText>
                  <HeroText layers={8} shadowColor="#000000" className="text-[120px] md:text-[200px] uppercase tracking-tighter leading-[0.85] text-white">
                    TOAST
...
const SocialLink = ({ href, children, title }: { href: string; children: React.ReactNode; title: string }) => (
  <a
    ...
    rel="noopener noreferrer"
    title={title}
    className="size-11 flex items-center justify-center bg-black rounded-2xl shadow-[3px_3px_0_rgba(255,255,255,0.25)] transition-all hover:-translate-y-1 active:translate-y-0 active:shadow-none"
  >
    {children}
  </a>
);
